...
import numpy as np
from nums import nums_to_divide, signs, forDivide, calc, check_sign
...
class Simple:
    ...
    def branch_examples(self):
        ...
        return ["".join([x for x in branches]), sum]
...
class Normal:
    ...
    def simple_equantion(self):
        ...
        #     while True:
        #         eq_signs = [signs[random.randint(0, 3)] for i in range(2)]
        #         if (eq_signs.count(":") == 1 or eq_signs.count("×") == 1) and (eq_signs.count(":") + eq_signs.count("×") < 3):
        #             break

        
def chech_answers(inp, answer, attempts=3):
    if inp == answer:
        return ["Верно!", True]
    else:
        if attempts == 0:
            return ["Неверно!", False]
        else:
            return [f"Неверно! Осталось попыток {attempts}/3", False]